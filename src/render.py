...
from data import CONTACT, FORMATION, EXPERIENCES, PROJET_ACADEMIQUE, LANGUES, CENTRES_INTERET, PROFILES
...
def _header_html():
    ...
    <div class="header">
      ...
      <div class="contact-row">
        ...
        <span>{CONTACT['tel']}</span><span>|</span>
...
def _experiences_html():
    ...
    return f'<h2 class="section">EXPERIENCES PROFESSIONNELLES</h2>{rows}'
...
def render_ats(profile: dict) -> str:
    comp_lines = "".join(
        f'<div><b>{label}</b> : {value}</div>' for label, value in profile["competences"]
    )
    langues_line = " &nbsp;•&nbsp; ".join(LANGUES)
    centres_line = " &nbsp;•&nbsp; ".join(CENTRES_INTERET)

    css = BASE_CSS + """
    .content { padding: 12px 34px 12px 34px; }
    .content p { text-align: justify; line-height: 1.35; margin: 3px 0; }
    h2.section { margin: 9px 0 6px 0; }
    .entry { margin-bottom: 6px; }
    .entry-etab { margin-bottom: 2px; }
    """
